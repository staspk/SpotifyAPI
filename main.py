...
"I want my Wife's true favorite songs, based on Her entire Spotify listening history, minus my Liked Songs"
    Assumes: you have already imported the necessary Spotify Data Types; ie:
        1. `Spotify Extended Streaming History` for `HER`, via: `py ./import_spotify_data.py Luna`
        2. `Spotify Account Data` for `ME`,                via: `py ./import_spotify_data.py Stan`
        
        if you're missing #2, check out SpotifyUser(HER).favorite_songs()
"""
ME = SpotifyUser('Stan')
HER = SpotifyUser('Luna')

her_favorite_songs = I_want_her_favorite_songs(HER, ME, minimum_listen_time_in_hours = 1)
...
"""
"I want to backup my Liked Songs (using Web Api)"
"""
SpotifyUser('Stan').Save_Liked_Songs()